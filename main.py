...
ZONES_CSV_URL   = os.getenv("ZONES_CSV_URL")   # CSV‑таблица зон (ID,Region) в UTF‑8
REES_SHEETS_MAP = {
...
def load_zones_map() -> dict:
    """Скачиваем CSV из ZONES_CSV_URL, декодируем как UTF‑8(+BOM), возвращаем { user_id: region }."""
    resp = requests.get(ZONES_CSV_URL)
    resp.raise_for_status()
    # decode explicitly as UTF-8 (strip BOM if any)
    text = resp.content.decode("utf-8-sig")
    df = pd.read_csv(StringIO(text), dtype=str)
    return dict(zip(df["ID"].str.strip(), df["Region"].str.strip()))

def load_data(excel_url: str) -> pd.DataFrame:
    """Скачиваем XLSX‑таблицу РЭС и возвращаем pandas.DataFrame."""
...
def handle_message(update: Update, context: CallbackContext):
    ...
    # 1) Если это нажатие кнопки с типом информации — сразу отобразить данные
    ...
    # 2) Иначе — пытаемся принять ввод за номер счётчика
    ...
    if region.upper() == "ALL":
        ...
        if not excel_url:
            return update.message.reply_text(f"Таблица для региона «{region}» не задана.")
    ...
    # 3) Сохраняем состояние и предлагаем кнопки
