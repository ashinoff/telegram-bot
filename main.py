...
def refresh_vols():
    ...
    try:
        ...
        # find the column containing "тп" (case-insensitive)
        for c in df.columns:
            if "тп" in c.lower():
                VOLS_TP_COL = c
                break
...
# === LOAD ZONES CSV (with dynamic detection of Region and Region_TP columns) ===
def load_zones_map():
    ...
    # find any column with "тп" in header
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if state.get("mode") == "vols_tp_input":
        ...
        return update.message.reply_text(
            f"На ТП {tp} найдено {len(df_tp)} договор(ов):",
