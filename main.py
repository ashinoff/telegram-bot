...
def load_zones_map():
    ...
    # ищем колонку Region ТП по вхождению 'тп'
...
def handle_message(update: Update, context: CallbackContext):
    ...
    if state.get("mode")=="info":
        ...
        # выбираем колонки в зависимости от кнопки
        ...
        else:  # "Информация по прибору учета"
            cols = [
    ...
    if state.get("mode")=="vols_tp_input":
        if txt=="Назад":
            ...
            return update.message.reply_text("Меню ВОЛС:", reply_markup=VOLS_MENU)
        tp = txt.upper()
        tp = tp if tp.startswith("ТП-") else f"ТП-{tp}"
        ser_tp  = VOLS_DF[VOLS_TP_COL].astype(str).str.upper().str.strip()
        mask_tp = ser_tp==tp
        if not mask_tp.any():
            return update.message.reply_text("Договоров нет.", reply_markup=ReplyKeyboardMarkup([["Назад","Новый поиск"]], resize_keyboard=True))
        df_tp = VOLS_DF[mask_tp]
        # фильтрация по region_tp
        if region_tp.upper()!="ALL":
            df_tp = df_tp[df_tp["РЭС"].astype(str).str.strip()==region_tp]
            if df_tp.empty:
                return update.message.reply_text("У вас нет доступа к этой зоне.", reply_markup=ReplyKeyboardMarkup([["Назад","Новый поиск"]], resize_keyboard=True))
        # считаем для контрагентов
        ...
        user_states[uid] = {
            "mode":"vols_tp_list","df_tp":df_tp,
            "label_map":label_map,"region_tp":region_tp
        }
        return update.message.reply_text(f"На ТП {tp} — {len(df_tp)} договор(ов):", reply_markup=ReplyKeyboardMarkup(buttons, resize_keyboard=True))
    ...
    if state.get("mode")=="vols_tp_list":
        ...
        if txt in lm:
            ...
            return update.message.reply_text("Выберите другого контрагента или Назад.", reply_markup=ReplyKeyboardMarkup([[b] for b in list(lm)+["Новый поиск","Назад"]], resize_keyboard=True))
    ...
    if state.get("mode")=="vols_provider_input":
        if txt=="Назад":
            ...
            return update.message.reply_text("Меню ВОЛС:", reply_markup=VOLS_MENU)
        provider = txt.strip()
        df_p = VOLS_DF[VOLS_DF["Наименование контрагента (собственника ВОЛС)"].str.contains(provider, case=False, na=False)]
        if region_tp.upper()!="ALL":
        ...
        return update.message.reply_text(f"Найдено в {len(df_p)} договор(ах):", reply_markup=ReplyKeyboardMarkup(buttons, resize_keyboard=True))
